...
class StepTracer:
    ...
    def trace_lines(self, frame, event, arg):
        ...
        # Only trace frames that belong to the compiled Saral program.
        # Without this check the tracer follows execution into Python stdlib
        # modules (urllib → email → string → re) where sre_parse.SubPattern
        # objects land in frame.f_locals and break _extract_vars on Python 3.12+.
        if frame.f_code.co_filename != self.session.filename:
            return None
...
AI_DEBUGGER_BANNER = """
╔══════════════════════════════════════════════╗
║   🤖  Saral AI Auto-Debugger v1.1           ║
║   Cloud AI analyses your program fully       ║
║   Runs automatically — no step prompts       ║
╚══════════════════════════════════════════════╝
"""


def run_debugger_ai(filepath: str):
    """
    AI Auto-debug mode — called by `saral --debugger` and auto-launched on error.
    Runs the program fully (no step-by-step prompts), then automatically
    triggers cloud AI analysis of the complete execution session.
    """
    ...
    print(AI_DEBUGGER_BANNER)
    print(_c(f"  File: {filepath}", C.CYAN) +
          _c(f"  ({len(source.splitlines())} lines)", C.DIM))
    print()
    print(_c("  🤖  AI Debugger is running your program automatically...", C.CYAN))
    print()

    _run_debug_session_ai(filepath, source)


def _run_debug_session_ai(filepath: str, source: str):
    """
    Compile and run one AI-automatic debug session.
    Always runs in 'run' mode (not step-by-step).
    Auto-triggers AI analysis on error AND on clean completion.
    Called by run_debugger_ai() and by the 'restart' command.
    """
    ...
    try:
        ...
        print(_c("❌  Compile errors above — AI cannot run until they are fixed.", C.RED))
        return
    ...
    if parse_errors:
        ...
        for e in parse_errors:
            print(f"    {e}")
        print()
        print(_c("  🤖  AI is analysing the compile errors...", C.CYAN))
        session = DebugSession(filepath, source)
        for err in parse_errors:
            session.record_error(0, "ParseError", err)
        _ai_fix(session)
        return
    ...
    session = DebugSession(filepath, source)
    tracer  = StepTracer(session, smap, step_mode=False)

    print(_c("  ── Running ─────────────────────────────", C.DIM))
    exec_globals = {"__name__": "__main__"}

    def _trace_call(frame, event, arg):
        if event == "call" and frame.f_code.co_filename == filepath:
            return tracer.trace_lines
        return None
    ...
    try:
        ...
        # Auto-trigger AI analysis even on clean completion
        print()
        print(_c("  🤖  AI is analysing your program execution...", C.CYAN))
        _ai_fix(session)
        ...
        print()
        print(_c("  🤖  AI is analysing the error automatically...", C.CYAN))
        _ai_fix(session)
    ...
    print(_c("  debug   ", C.GREEN) + "— switch to interactive step-by-step debugger")
    print(_c("  restart ", C.GREEN) + "— run AI debugger again from the start")
    ...
    while True:
        ...
        if cmd in ("quit", "q", "exit"):
        ...
        elif cmd in ("debug", "step", "interactive"):
            print(_c("\n  Switching to interactive step debugger...\n", C.CYAN))
            _run_debug_session(filepath, source)
            break
        elif cmd in ("restart", "r"):
            print(_c("\n  Restarting AI debug session...\n", C.CYAN))
            _run_debug_session_ai(filepath, source)
            break
        elif cmd in ("list", "l"):
            ...
            print(_c(f"  Unknown: {cmd}", C.DIM) +
                  "  (quit / fix / history / vars / save / debug / restart)")
...
def _run_debug_session(filepath: str, source: str):
    ...
    try:
        ...
        print(_c("❌  Compile errors above.", C.RED))
        print()
        print(_c("  🤖  AI is analysing the compile errors...", C.CYAN))
        _session = DebugSession(filepath, source)
        _session.record_error(0, "CompileError", "Program could not be compiled")
        _ai_fix(_session)
        return
    ...
    if parse_errors:
        ...
        for e in parse_errors:
            print(f"    {e}")
        print()
        print(_c("  🤖  AI is analysing the parse errors...", C.CYAN))
        _session = DebugSession(filepath, source)
        for err in parse_errors:
            _session.record_error(0, "ParseError", err)
        _ai_fix(_session)
        return
